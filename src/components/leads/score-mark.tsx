...
const CHIP: Record<HeatLevel, string> = {
  hot: "border-[rgba(232,189,54,0.6)] bg-[linear-gradient(180deg,#fff3c4,#efc94a)] text-[var(--spm-navy)] shadow-[0_0_0_2px_rgba(232,189,54,0.18)]",
    ...
    <span className="inline-flex flex-col gap-0.5">
      <span
        className={cn(
          ...
          CHIP[level],
          compact
            ? "px-1.5 py-px text-[9px] font-extrabold tracking-[0.06em]"
            : "px-2 py-0.5 text-[10px] font-extrabold tracking-[0.08em]",
        )}
      >
        {LEVEL_LABEL[level]}
      </span>
      <span className="flex w-full min-w-[2.75rem] gap-px" aria-hidden>
        {[1, 2, 3].map((tick) => (
          <span
            key={tick}
            className={cn(
